...
df_filtered.to_csv('resultado_control_row1.tsv', sep='\t')

palette_base = sns.color_palette("Greys", n_colors=5)
...
for line, name in zip(ax.lines, df['Time'].unique()):
    x = line.get_xdata()[0]      # primera coordenada x
    y = line.get_ydata()[0]      # primera coordenada y

    # Mover solo la etiqueta "2h"
    if name == '2h':
        ax.text(x - 3.7, y - 0.05, name,   # ↑ mueve un poco más arriba
...
plt.ylim(0, 1.2)
plt.annotate('Control',
             xy=(50, 0.8),          # punto que quieres señalar (x, y)
             xytext=(60, 1.07),      # posición del texto
             color='black',fontsize=12
             )
plt.grid(False)
plt.savefig("graph_control_row1.png",
            dpi=300,
            bbox_inches='tight',
            transparent=True)
plt.show()